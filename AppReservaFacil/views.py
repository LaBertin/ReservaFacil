...
def especialista_list_citas(request):
    ...
    if request.method == 'POST':
        if 'doc_pac' in request.POST:
            ...
            if Ficha_Medica.objects.filter(Nombre_Com_Pac = nom_pac).exists():
                Ficha_Medica_Pac = Ficha_Medica.objects.get(Nombre_Com_Pac = nom_pac).ID_Ficha_Medica
            else:
                Ficha_Medica_Pac = None
                messages.warning(request,f"¡Atención! {nom_pac} no cuenta con una ficha médica.")
            ...
            url = reverse('agregar_cita_medica') + '?Ficha_Medica_Pac={}&RUT_Pac={}&Nombre_Com_Pac={}'.format(
                Ficha_Medica_Pac, RUT_Pac, nom_pac)
            return redirect(url)
        if 'conf_delet_cit' in request.POST:
